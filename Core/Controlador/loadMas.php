...
if(isset($_POST['loadMas'])){
    $connection = new Conection();
    $consultas  = new Consultations();
    $connect    = $connection->get_conection();

    $messages   = array();
    $id_conver  = (int)htmlentities(addslashes($_POST['convers']));
    $online     = (int)htmlentities(addslashes($_POST['online']));
    $topid      = (int)htmlentities(addslashes($_POST['topid']));

    $persConver = $connect->prepare("SELECT * FROM `messages` WHERE ((`id_de` = ? AND `id_para` = ?) OR (`id_de` = ? AND `id_para` = ?)) AND (`id` < $topid) ORDER BY `id` DESC LIMIT 10");
    $persConver->execute(array($online, $id_conver, $id_conver, $online));

    while($row = $persConver->fetch()){
        $fotouser = '';
        if($online == $row['id_de']){
