...
require_once '../Modelo/class.consultations.php';
    ...
    echo '<p class="flaticon-document" style="color: #fff;"> Comentariós: [';
    if(isset($MessageId)){
        echo $MessageId;
    }
    echo ']</p>';

    echo "<p style='color: #fff;' class='text-center'>".date('Y/F/l-d h:i:s A')."</p>";

    if(isset($conversations)){
        foreach($conversations as $conversation){
            $msj     = nl2br($conversation['message']);
            $targets = $modelo->link($msj);
            $newMsj  = wordwrap($targets, 25, "\n", true);
            ...
            if(isset($_SESSION['id_user'], $conversation['id_users']) && $conversation['id_users'] == $_SESSION['id_user']){
                ...
                echo "<h3 class='text-right'>
                        <a style='color: #fff' href='../Acceso/Usuarios.php?id_users=".$conversation['id_users']."'>
                            ".$conversation['user_name']."
                        </a>
                      </h3>";
                echo "<span style='font-size: 12px;color: #fff;' class='pull-right'>".strtoupper($conversation['date_message'])."</span><br>";

                echo '<p class="postRight">'.$newMsj."\n";
                ...
                echo "<h3 class='text-left'>
                        <a style='color: #ffffff' href='../Acceso/Usuarios.php?id_users=".$conversation['id_users']."'>
                            ".$conversation['user_name']."
                        </a>
                      </h3>";
                echo "<span style='font-size: 12px;' class='pull-left'>".strtoupper($conversation['date_message'])."</span><br>";

                echo '<p class="posttwoa">'.$newMsj."\n";
                if($session[0]['rango'] >= 2){
